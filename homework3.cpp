...
public:
    ...
    public:
        reference operator*() {
            ...
            int ind = (parent->begin_index + offset) % MAX_SIZE;
            return parent->buffer[ind];
        }
...
private:
    ...
    int end_index() const {
        // Replace the line(s) below with your code.
        int endInd = (begin_index+ring_size)%MAX_SIZE;
        return endInd;
    }
...
public:
    ...
    ItemType back() const {
        ...
        // Replace the line(s) below with your code.
        int lastElement = (begin_index+ring_size-1)%MAX_SIZE;
    ...
    void push_back( const ItemType& value ){
        ...
        if (ring_size != MAX_SIZE) //Capacity not reached
            ring_size++;
        else //Capacity reached
            increment_begin_index();
        return;
    }
...
int main(){
    ...
    rq.pop_front();rq.pop_front();rq.pop_front();rq.pop_front();rq.pop_front();
    for ( int i = 0 ; i < 3; i++ )
        rq.push_back(i+101);
    //rq.pop_front();rq.pop_front();rq.pop_front();
    /*
    rq.pop_front();
    rq.pop_front();
    rq.pop_front();
    ...
    */
    ...
    //rq.dump_queue();
    std::cout << rq.front() << " " << rq.back() << std::endl;
